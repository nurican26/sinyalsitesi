...
# Sayfa Tasarım Ayarları - GENİŞ EKRAN DÜZENİ
...
# 🤬 YASAKLI KELİMELER / KÜFÜR FİLTRESİ LİSTESİ
# (Buraya engellemek istediğiniz kelimeleri küçük harflerle ekleyebilirsiniz)
YASAKLI_KELIMELER = ["küfür1", "küfür2", "hakaret1", "argokelime", "salak", "aptal"]
...
with sol_taraf:
                    ...
                    for i in range(len(df)):
                        ...
                        if "[AL]" in w_sutun_verisi or "AL" in w_sutun_verisi:
                            ...
                            if not hisse_data.empty:
                                ...
                                # O anlık canlı fiyatı ve zamanı kalıcı olarak kutuya kilitler
    ...
    # 📦 YENİ SÜTUNLU ÖZEL KAYIT KUTUSU (TAKİP SİSTEMİ)
    if st.session_state["ozel_takip_kutusu"]:
        ...
        for hisse, bilge in list(st.session_state["ozel_takip_kutusu"].items()):
            ...
            if not hisse_data.empty:
                ...
                # Anlık kar zarar oranının hesaplanması sütunu
                ...
                kutu_tablo.append({
                    "Hisse Kodu": hisse, 
                    "Kutuya Kayıt Fiyatı (O Anlık)": f"{eski_fiyat:.2f} TL", 
...
with sag_taraf:
    ...
    # 🔒 YÖNETİCİ PANELİ (nurican veya Nurican yazıldığında oda sayısı ikonu ve kısıtlama açılır)
    if isat.strip().lower() == "nurican":
        st.markdown(f"### 🚪 Oda Sayısı: {st.session_state['oda_sayisi']} | 👑 Yetkili Girişi Onaylandı.")
