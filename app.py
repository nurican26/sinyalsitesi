...
# Sayfa Yapılandırması (Hafif Sürüm)
st.set_page_config(page_title="BTA Finans", layout="wide")
...
# EKSİK OLAN HAYATİ PARÇA: Sohbeti ve borsa ekranını 5 saniyede bir otomatik eşitler ve yeniler!
st_autorefresh(interval=5 * 1000, key="bta_sohbet_anlik_senkronize_motoru")

excel_yolu = "nurican.xls.xlsm"

# --- KASMAYAN GÜVENLİ SOHBET HAFIZASI ---
if "bta_canli_sohbet_hafizasi" not in st.session_state:
    st.session_state["bta_canli_sohbet_hafizasi"] = [
        {"isim": "Ahmet Y.", "saat": "12:15", "yorum": "Sistem anlık yenileme motoruyla artık roket gibi, elinize sağlık."},
        {"isim": "Sistem", "saat": "10:00", "yorum": "BTA Canlı Sohbet Alanı Aktif."}
    ]
...
# ===================================================================== #
# 5. DOĞRUDAN VE ANINDA GÜNCELLEYEN CANLI SOHBET SİSTEMİ
# ===================================================================== #
st.write("---")
st.subheader("💬 KULLANICI YORUMLARI VE CANLI SOHBET")

with st.form(key="bta_canli_sohbet_formu", clear_on_submit=True):
    y_is = st.text_input("Adınız / Rumuzunuz:", max_chars=25)
    y_me = st.text_area("Mesajınız:", max_chars=300, height=80)
    bta_gonder_tusu = st.form_submit_button("Mesajı Yayınla 📨", use_container_width=True)
    ...
    if bta_gonder_tusu:
        if y_is.strip() and y_me.strip():
            ...
            if not any(z in m_kucuk for z in ["orospu", "amk", "oç", "oc", "siktir", "piç", "salak"]):
                ...
                st.session_state["bta_canli_sohbet_hafizasi"].insert(0, y_satir)
                st.rerun()
            else:
...
# MESAJ LİSTELEME
for s, sh in enumerate(st.session_state["bta_canli_sohbet_hafizasi"]):
    st.write(f"👤 **{sh['isim']}** ({sh['saat']}): {sh['yorum']}")
    if adm_mod:
        if st.button(f"Sil ❌ (Sıra: {s+1})", key=f"sl_{s}"):
            st.session_state["bta_canli_sohbet_hafizasi"].pop(s)
            st.rerun()
