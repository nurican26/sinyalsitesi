...
st.markdown('<style>.stApp {background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%)!important; padding: 0.5rem;} h1,h2,h3,h4,h5,h6,p,span,label {color: #fff!important; font-family: "Segoe UI", sans-serif;} input {color: #000!important; background-color: #fff!important;} .stDataFrame {width: 100% !important; border: 1px solid #10b981 !important; border-radius: 8px;} div.block-container {padding-top: 1rem; padding-bottom: 0.5rem;} .alsat-baslik {background: linear-gradient(90deg, #ca8a04 0%, #1e1b4b 100%); padding: 8px; border-radius: 5px; font-weight: bold; margin-bottom: 5px;} .al-baslik {background: linear-gradient(90deg, #16a34a 0%, #1e1b4b 100%); padding: 8px; border-radius: 5px; font-weight: bold; margin-bottom: 5px;} .spk-kutusu {background-color: rgba(220, 38, 38, 0.15) !important; border: 1px solid #dc2626 !important; padding: 15px !important; border-radius: 6px !important; margin-top: 30px !important; margin-bottom: 20px !important; color: #fca5a5 !important; font-size: 0.9rem !important; text-align: justify !important; line-height: 1.5 !important; display: block !important;} div[data-testid="stDataFrame"] td, div[data-testid="stDataFrame"] th {font-size: 1.25rem !important; font-weight: bold !important; color: #ffffff !important;} .piyasa-kutusu {background: rgba(255, 255, 255, 0.05); border: 1px solid #eab308; padding: 10px; border-radius: 8px; text-align: center; font-weight: bold;} .haber-kutusu {background: rgba(255, 255, 255, 0.03); border-left: 4px solid #10b981; padding: 12px; border-radius: 6px; margin-bottom: 10px;}'
            '/* 🌟 3D DERİN GÖLGELİ VE HAREKETLİ YENİ BTA LOGO TASARIMI */'
            '.bta-marka-alani {text-align: left; margin-top: 10px; margin-bottom: 30px;}'
            '.bta-logo-yeni {font-size: 3.5rem; font-weight: 900; letter-spacing: 2px; background: linear-gradient(135deg, #10b981 0%, #059669 50%, #eab308 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; filter: drop-shadow(2px 4px 10px rgba(16, 185, 129, 0.5)); display: inline-block; font-family: "Impact", "Arial Black", sans-serif; text-shadow: 0px 15px 20px rgba(0,0,0,0.7);}'
            '.bta-alt-yazi {font-size: 1.1rem; color: #a7f3d0 !important; font-weight: 600; letter-spacing: 1px; margin-top: -5px; opacity: 0.9; text-shadow: 1px 1px 3px rgba(0,0,0,0.5); font-style: italic;}</style>', unsafe_allow_html=True)
        ...
        try:
            if len(df_kaynak.columns) > 22:
                ...
                if uv and uv not in ["NAN", "NONE", "AL_SAT SİNYALİ"]:
                    ...
                    if h_ara:
                        hisse = str(h_ara).strip() # 🛠️ PARANTEZLER İMHA EDİLDİ (KESİN DÜZ HARF)
...
# ⚖️ MUTLAK SABİT YASAL UYARI KUTUSU
st.write("---")
