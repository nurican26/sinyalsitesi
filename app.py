...
# CSS Düzenlemeleri (Tablo yazı boyutları ve mobil uyumluluk optimize edildi)
st.markdown("""
<style>
    .stApp {
...
# String operasyonlarındaki hatalı replace mantığı f-string formatlamasıyla düzeltildi
c1.markdown(f'<div class="piyasa-kutusu">🔱 GRAM ALTIN<br><span style="color:#eab308; font-size:1.4rem;">{p_gram:,.2f} TL</span></div>'.replace(',', 'X').replace('.', ',').replace('X', '.'), unsafe_allow_html=True)
c2.markdown(f'<div class="piyasa-kutusu">🪙 ÇEYREK ALTIN<br><span style="color:#eab308; font-size:1.4rem;">{p_ceyrek:,.2f} TL</span></div>'.replace(',', 'X').replace('.', ',').replace('X', '.'), unsafe_allow_html=True)
c3.markdown(f'<div class="piyasa-kutusu">🥈 YARIM ALTIN<br><span style="color:#eab308; font-size:1.4rem;">{p_yarim:,.2f} TL</span></div>'.replace(',', 'X').replace('.', ',').replace('X', '.'), unsafe_allow_html=True)
c4.markdown(f'<div class="piyasa-kutusu">🥇 TAM ALTIN<br><span style="color:#eab308; font-size:1.4rem;">{p_tam:,.2f} TL</span></div>'.replace(',', 'X').replace('.', ',').replace('X', '.'), unsafe_allow_html=True)
st.write("")
                ...
                if uv and uv not in ["NAN", "NONE", "AL_SAT SİNYALİ"]:
                    ...
                    if h_ara:
                        ...
                        tablo_alsat.append({
                            "Hisse Kodu 📈": hisse, 
                            "BTA Puan": bta_puan, 
                            "💥 İnternet Canlı": f"{cfiy:,.2f} TL".replace(',', 'X').replace('.', ',').replace('X', '.') if cfiy > 0 else "Yükleniyor..."
                        })
...
st.markdown('<div class="haber-kutusu">🌟 <b>Altın Piyasası:</b> Ons altın ve iç piyasada döviz kurlarının dengelenmesiyle gram ve çeyrek altın dinamikleri yakından izleniyor.</div>', unsafe_allow_html=True)
