...
# 🔑 PARAMETRELER
YONETICI_SIFRESI = "bta2026"  # Sol menüyü açıp odayı kilitleyip açmak için senin şifren
MESAJ_DOSYASI = "gelen_mesajlar.txt"

# Hafıza Kontrolleri (Oturum Durumları)
if "oda_kilitli_mi" not in st.session_state:
    st.session_state["oda_kilitli_mi"] = False  # İlk açılışta herkese açık başlar
...
# 🛠️ SOL MENÜ: YÖNETİCİ KONTROL ODASI
...
# Eğer yönetici şifresi doğruysa Kilitleme / Açma butonları görünür
...
st.sidebar.divider()
st.sidebar.info("Bu menü sadece admin tarafından odayı kilitlemek veya açmak için kullanılır.")


# --- 🏢 DURUM KONTROLÜ VE İÇERİK ---

# 1. DURUM: ODA KİLİTLİYSE VE YÖNETİCİ GİRİŞİ YAPILMAMIŞSA
if st.session_state["oda_kilitli_mi"] and admin_sifre != YONETICI_SIFRESI:
    st.markdown('<div class="kilit-uyari">🔒 <b>BTA Odası Geçici Olarak Kilitlenmiştir!</b><br>Analizler ve sinyal listesi şu an bakımdadır veya erişime kapatılmıştır. Lütfen daha sonra tekrar deneyiniz.</div>', unsafe_allow_html=True)
    
    # Kilitliyken sadece sayaçlar görünür
    ...
    with col1: st.metric(label="📊 Toplam Panel Girişi", value=st.session_state["ziyaret_sayaci"])
    with col2: st.metric(label="🔥 Verilen Toplam Oy", value=st.session_state["topham_oy_sayisi"])
    with col3: st.metric(label="⭐ Panel Puanı", value=f"{puan:.2f} / 5")

# 2. DURUM: ODA HERKESE AÇIKSA VEYA YÖNETİCİ KENDİSİ BAKIYORSA
else:
    if st.session_state["oda_kilitli_mi"]:
        st.warning("⚠️ Oda şu an diğer kullanıcılara kilitli ancak Yönetici olduğunuz için içeriği görüyorsunuz.")
    ...
    if os.path.exists(excel_yolu):
        ...
        st.warning(f"⚠️ '{excel_yolu}' dosyası bulunamadı. Lütfen dosya adını kontrol edin.")
    ...
    else: st.write("📈 Sinyaller taranıyor...")
