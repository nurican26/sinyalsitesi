...
# ⚖️ MUTLAK SABİT YASAL UYARI KUTUSU (En üstte, logonun hemen altında asla silinmez!)
    ...
    for idx in range(2, len(df_kaynak)):
        try:
            if len(df_kaynak.columns) > 22:
                ...
                # 🛠️ GÜVENLİ VE TAM KELİME PARANTEZSİZ AYIKLAMA MOTORU
                if uv and uv not in ["NAN", "NONE", "AL_SAT SİNYALİ"]:
                    uv_temiz = "".join([c for c in uv if c.isalnum() or c == "."])
                    h_ara = re.findall(r'[A-Z]+', uv_temiz)
                    if h_ara:
                        hisse_eslesme = str(h_ara[0]).strip() # Listenin ilk elemanını tam kelime düz yazı olarak çeker
