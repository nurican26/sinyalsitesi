...
with tab_members:
    ...
    with st.form("member_form", clear_on_submit=True):
        ...
        with col1:
            hissedar_adi = st.text_input("Hissedar Adı:", placeholder="Örn: Nurican Bey")
            hisse_kodu = st.text_input("Sahip Olduğu BTA Hisse Kodu:", placeholder="Örn: KONYA.IS", value="KONYA.IS")
        with col2:
            hisse_maliyeti = st.number_input("Hisse Maliyeti (TL):", min_value=0.0, step=100.0)
            hisse_adeti = st.number_input("Hisse Adet:", min_value=1, step=1)
        
        submit_member = st.form_submit_button("Hissedarı Kaydet ✅")
        if submit_member and hissedar_adi:
            ...
            st.session_state["bta_members_list"].append(yeni_hissedar)
            st.success(f"✅ {hissedar_adi} başarıyla kayıt edildi!")
            st.rerun()
    
    st.markdown("---")
    
    # Kayıtlı Hissedarlar Listesi
    ...
    if st.session_state["bta_members_list"]:
        ...
        st.dataframe(df_members[["Hissedar Adı", "Sahip Olduğu BTA Hissesi", "Hisse Maliyeti (TL)", "Adet"]], use_container_width=True)
        
        # Yönetici Silme İşlemi
        if is_admin:
            st.subheader("🗑️ Yönetici: Hissedar Silme")
            silinecek_hissedar = st.selectbox(
                "Silinecek Hissedarı Seçin:",
                options=[f"{m['Hissedar Adı']} - {m['Sahip Olduğu BTA Hissesi']}" for m in st.session_state["bta_members_list"]],
                key="delete_member_select"
            )
            if st.button("Hissedarı Sil ❌"):
                secili_index = [i for i, m in enumerate(st.session_state["bta_members_list"]) 
                               if f"{m['Hissedar Adı']} - {m['Sahip Olduğu BTA Hissesi']}" == silinecek_hissedar][0]
                silinen = st.session_state["bta_members_list"].pop(secili_index)
                st.warning(f"🗑️ {silinen['Hissedar Adı']} kaydı silindi!")
                st.rerun()
    else:
        st.info("💡 Henüz kayıtlı hissedar bulunmamaktadır.")
