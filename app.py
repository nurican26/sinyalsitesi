...
if not os.path.exists(db_istatistik):
    pd.DataFrame([{"ziyaret_sayisi": 194, "basarili_oy": 15, "basarisiz_oy": 2}]).to_csv(db_istatistik, index=False)
...
try:
    ...
    if df_ist.empty:
        df_ist = pd.DataFrame([{"ziyaret_sayisi": 194, "basarili_oy": 15, "basarisiz_oy": 2}])
    ...
    ziyaret, basarili, basarisiz = 194, 15, 2

toplam_oy = basarili + basarisiz
basari_orani = int((basarili / toplam_oy) * 100) if toplam_oy > 0 else 93
...
# Mevcut kaydedilmiş geçmiş verileri CSV'den yükle
try:
    ...
    try:
        ...
        for idx in range(min(10, len(df))):
            ...
            if ha != "" and ha not in ["BTA HİSSE", "HİSSE", "NAN", "NONE", "ANA", "RAYSG"]:
                ...
                # Geçmişe kaydetmek için sadece Saat/Dakika filtresi yerine genel kontrol ekledik
                mukkerrer_kontrol = df_gecmis_db[(df_gecmis_db["Tarih"].str.contains(excel_guncelleme_tarihi.split(" -")[0])) & (df_gecmis_db["Hisse"] == ha)]
                if mukkerrer_kontrol.empty:
        ...
        # Yeni taranan veriyi hafızaya al
        if yeni_kayitlar:
        ...
        st.error(f"Excel tarama hatası: {e}")
