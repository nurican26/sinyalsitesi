    ...
    if os.path.exists(MESAJ_DOSYASI):
        ...
        try:
            ...
            if m_liste:
                ...
                st.write("")
                if st.button("🗑️ Tüm Mesajları Temizle", use_container_width=True): os.remove(MESAJ_DOSYASI); st.rerun()
        except: pass
...
if erisim_izni:
    ...
    # 📰 CANLI HABER AKIŞI (Harika Fikir Düzenlemesi)
    st.markdown("#### 📰 Canlı Ekonomi ve Borsa Gündemi")
    try:
        # Borsa İstanbul ve genel makro haber başlıklarını otomatik besler
        ...
        if bist_news:
            ...
            st.write("📰 Güncel finans haberleri taranıyor...")
    except:
        st.write("📰 Güncel finans haberleri taranıyor...")
    st.write("")
