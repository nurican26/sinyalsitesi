...
# 1. SAYFA YAPILANDURMASI VE TELEFON UYUMLU SÜPER NEON TASARIM
...
if secilen_panel == "🔎 Tüm BIST Arama Motoru":
    ...
    if arama_girdisi in bist_all:
        hisse_df = yf.download(f"{arama_girdisi}.IS", period="1d", progress=False)
        if not hisse_df.empty:
            # 🎯 TÜM COKLU BAŞLIK (MULTIINDEX) YAPILARINI BYPASS EDEN EN SAĞLAM METOD
            hisse_df.columns = ['_'.join(c) if isinstance(col, tuple) else str(c) for c in hisse_df.columns]
            fiyat_kolonlari = [col for col in hisse_df.columns if "Close" in col]
            if fiyat_kolonlari:
                son_fiyat = float(hisse_df[fiyat_kolonlari[0]].iloc[-1])
                st.success(f"📈 **{arama_girdisi}** Hissesi Başarıyla Bulundu!")
                st.metric(label="Anlık Canlı Fiyat (TL)", value=f"{son_fiyat:,.2f} TL")
...
if secilen_panel == "🪙 Canlı Altın Takibi":
    ...
    if not altin_download.empty:
        # 🎯 SIFIR SÖZDİZİMİ RISKLI EN YALIN OKUMA SEVİYESİ
        altin_download.columns = ['_'.join(col) if isinstance(col, tuple) else str(col) for col in altin_download.columns]
        ons_kolon = [c for c in altin_download.columns if "Close" in c and "GC=F" in c]
        usd_kolon = [c for c in altin_download.columns if "Close" in c and "TRY=X" in c]
        
        if ons_kolon and usd_kolon:
            ons_v = float(altin_download[ons_kolon[0]].iloc[-1])
