...
# 🔑 PARAMETRELER (Şifre alanları kaldırıldı)
...
# Üst Sayaç ve Bilgi Paneli (Herkes görebilir)
        ...
        try:
            if len(df_kaynak.columns) > 22:
                ...
                if uv_degeri and uv_degeri not in ["NAN", "NONE", "AL_SAT SİNYALİ"]:
                    ...
                    if hisse_ara:
                        hisse = hisse_ara[0] # Listeden çıkartıp sadece ismi alarak parantezleri temizler
...
# --- TABLOLAR DOĞRUDAN HERKESE GÖSTERİLİR ---
st.markdown('<div class="alsat-baslik">🟡 DÖNEMSEL AL SAT SİNYALLERİ</div>', unsafe_allow_html=True)
if tablo_alsat: 
    st.dataframe(pd.DataFrame(tablo_alsat), use_container_width=True, hide_index=True)
else: 
    st.write("📈 Sinyaller taranıyor...")
