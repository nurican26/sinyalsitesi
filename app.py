...
# 💾 Excel Okuma Önbelleği
@st.cache_data(ttl=300)
def excel_veri_yukle(dosya_yolu):
    if os.path.exists(dosya_yolu):
        try:
            return pd.read_excel(dosya_yolu, header=None, engine="openpyxl")
        except:
            return None
    return None

# 🟢 ERİŞİM İZNİ GEÇERLİYSE ÇALIŞACAK ALAN
if erisim_izni:
    ...
    df_kaynak = excel_veri_yukle(excel_yolu)

    # ⏳ Otomatik Yenileme Parçası (30 Saniyede Bir)
    @st.fragment(run_every=30)
    def sinyal_paneli_goster(df):
            ...
            for idx in range(2, len(df)):
                try:
                    if len(df.columns) > 22:
                        uv = str(df.iloc[idx, 20]).strip().upper() if not pd.isna(df.iloc[idx, 20]) else ""
                        wv = str(df.iloc[idx, 22]).strip().upper() if not pd.isna(df.iloc[idx, 22]) else ""
                        # 19. İndeks Excel'deki T sütununu doğrudan metin olarak okur
                        t_deg = str(df.iloc[idx, 19]).strip() if not pd.isna(df.iloc[idx, 19]) else ""
        ...
        st.caption(f"⏱️ Son Fiyat Güncellemesi: {datetime.datetime.now().strftime('%H:%M:%S')} (Her 30 saniyede bir otomatik yenilenir)")

    sinyal_paneli_goster(df_kaynak)
