...
# 1. Sayfa Yapılandırması ve Geniş Ekran Tasarımı
st.set_page_config(page_title="BTA", page_icon="📈", layout="wide")

# CSS Tasarımı - Sitenin Görsel Kimliği ve Renkleri
st.markdown("""
<style>
...
# Hafıza Kontrolleri (Arama yapınca verilerin silinmesini engeller)
...
admin_sifre = st.sidebar.text_input("Yönetici Şifresi:", type="password", placeholder="Ayarlar için girin...", key="admin_sifre_input_bta_yeni")
...
if admin_sifre == YONETICI_SIFRESI:
    ...
    if st.session_state["oda_kilitli_mi"]:
        ...
        if st.sidebar.button("🔓 Odayı Herkese Aç", use_container_width=True, key="oda_ac_btn_bta_yeni"):
            st.session_state["oda_kilitli_mi"] = False
            st.rerun()
    else:
        st.sidebar.success("🟢 Şu an: HERKESE AÇIK")
        if st.sidebar.button("🔒 Odayı Herkese Kilitle", use_container_width=True, key="oda_kilit_btn_bta_yeni"):
