...
# 👤 NICKNAME TABANLI ÜYE SİSTEMİ HAFIZASI
if "uye_nick" not in st.session_state:
    st.session_state["uye_nick"] = None
...
# ==========================================
# 👤 ÜYE GİRİŞ MODÜLÜ (NICKNAME SİSTEMİ)
# ==========================================
if st.session_state["uye_nick"] is None:
    st.header("🔐 Platforma Giriş Yapın")
    st.info("Sistemdeki verileri, canlı odaları ve sohbeti görebilmek için lütfen kendinize bir takma ad belirleyin.")
    
    with st.form("giris_formu"):
        girilen_nick = st.text_input("Takma Adınız (Nick):", placeholder="Örn: Nurican26")
        buton_giris = st.form_submit_button("Sisteme Giriş Yap 🚀")
        
        if buton_giris:
            if girilen_nick.strip() != "":
                st.session_state["uye_nick"] = girilen_nick.strip()
                st.success(f"Hoş geldiniz, {st.session_state['uye_nick']}! Platform aktif edildi.")
                st.rerun()
            else:
                st.error("Lütfen boş bir takma ad girmeyin!")
else:
    # Kullanıcı giriş yaptıysa üst barda durumunu ve Çıkış butonunu gösterelim
    col_user1, col_user2 = st.columns([0.85, 0.15])
    with col_user1:
        st.markdown(f"🟢 **Aktif Kullanıcı:** `{st.session_state['uye_nick']}`")
    with col_user2:
    ...
    # Sekmeli Menü Tasarımı (Sadece giriş yapıldığında görünür)
    ...
    with tab_chat:
        ...
        st.write("Sohbet odası herkese açıktır. Giriş yaptığınız takma ad otomatik kullanılır.")
