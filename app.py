...
# 🎯 GÜVENLİ İNTERAKTİF SEÇİCİ PANELİ
st.sidebar.markdown("---")
...
# --- 🚀 SEÇİLEN PANELİN EKRANA BASILMASI ---
        ...
        try:
            ...
            if not h_data.empty:
                ...
                for col in h_data.columns:
                    if "Close" in str(col):
                        c_price = float(last_row[col].values)
                        break
                if c_price > 0:
...
if secilen_panel == "🪙 Canlı Altın Takibi":
    ...
    try:
        ...
        if not altin_download.empty:
            last_row_a = altin_download.tail(1)
            ons_v, usd_v = 0.0, 0.0
            for col in altin_download.columns:
                if "Close" in str(col) and "GC=F" in str(col): ons_v = float(last_row_a[col].values)
